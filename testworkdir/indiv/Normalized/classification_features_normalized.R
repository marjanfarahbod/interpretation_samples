...
data <- read.delim("./indiv/Normalized/classification_features_normalized.tab", header=TRUE)
...
ggsave(paste("indiv/Normalized/classification_features_normalized_ecdf.pdf", sep=""), p, width=10, height=6, units="in")
...
ggsave(paste("indiv/Normalized/Clusters/classification_features_normalized_",i,".pdf", sep=""), p)
}
...
p <- ggplot(data[data$celltype==i,], aes(x=bio_label, y=feature_name, fill=feature_val_disc)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90))
ggsave(paste("indiv/Normalized/Bio-labels/classification_features_normalized_",i,".pdf", sep=""), p)
}
...
p <- ggplot(data, aes(y=label_celltype_key, x=feature_name, fill=feature_val_disc)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("indiv/Normalized/classification_features_normalized_all.pdf", p, width=15, height=350, units="in", limitsize=FALSE)
